<template>
    <div>
        <div id="pageTop">
            ...
            <div id="pageTopTable">
                <h3 id="TitleSenateTable">Senate at a glance</h3>
...
export default {
  ...
  secondTable(member) {
      const minVotes = []
      member.forEach(member => {
        minVotes.push(member.missed_votes_pct)
      })
    const percent = Math.floor(member.length/100*10)
    const minVote = minVotes.sort((a,b)=> b-a).slice(0, percent)
    const minValue = Math.min.apply(Math, minVote)
    
    function mountTable () {
    const test = []
    member.forEach(member => {
        if (member.missed_votes_pct >= minValue) {
            test.push(member)
        }
    })
    return test
    }
    this.secTableTotal = mountTable()
  },
  thirdTable(member) {
      const maxVotes = []
      member.forEach(member => {
          maxVotes.push(member.missed_votes_pct)
      })
    
    const percent = Math.floor(member.length/100*10)
      const maxVotesOrg = maxVotes.sort((a,b)=> a-b).slice(0, percent)
      const minValue = Math.max.apply(Math, maxVotesOrg)
